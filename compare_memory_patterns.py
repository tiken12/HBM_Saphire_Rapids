    ...
    with open(filename, newline="") as f:
        ...
        for row in reader:
            ...
            y.append(float(row[2])
                     if 'bandwidth' in filename else float(row[1]))
    return x, y


# Load data
# Kenneth: These are not like-for-like tests, so it probably doesn't make sense to put them on the same chart.
...
plt.plot(pointer_x, [1.0 / t for t in pointer_y],
         label='Pointer Chase (Est. Accesses/sec)', marker='^')
